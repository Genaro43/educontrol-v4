...
export default function AdminDashboard() {
    ...
    const handleGuardarUsuario = async () => {
        ...
        if (nuevaPassword.length < 6) {
            return mostrarNotificacion("La contraseña debe tener al menos 6 caracteres.", "advertencia");
        }
        ...
        if (authError) {
            if (authError.message.includes('already been registered')) {
                mostrarNotificacion(`El usuario "${nuevoUsuario}" ya existe. Elige otro nombre.`, "error");
            } else {
                mostrarNotificacion("Error de seguridad: " + authError.message, "error");
            }
            return;
        }
        ...
        if (perfilError) {
            ...
            // En lugar de alert(), mostramos la pantalla de éxito bonita
            setCredencialesGeneradas({ usuario: correoFantasma, password: nuevaPassword });

            // Limpiar formulario por debajo
            setNuevoUsuario('');
            setNuevoNombre('');
            setNuevaPassword('');
            cargarDatosPersonal();
    ...
    const manejarCargaArchivo = async (e) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                setMensajeUpload({ texto: `¡Éxito! Se procesaron ${alumnosAInsertar.length} alumnos correctamente.`, tipo: 'exito' });
    ...
    return (
        <div className="flex flex-col md:flex-row min-h-screen bg-[#f3f4f6] font-sans relative">
            ...
            {/* NOTIFICACIONES TOAST (Sustituto de alerts) */}
            {toast.visible && (
          ...
          ${toast.tipo === 'error' ? 'bg-white border-red-500 text-red-800' :
                ...
                >
                    {toast.tipo === 'error' && <svg className="w-6 h-6 text-red-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                    {toast.tipo === 'advertencia' && <svg className="w-6 h-6 text-[#F26522] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>}
                    <div>
                        <p className="font-bold text-sm text-gray-800">{toast.mensaje}</p>
                    </div>
                ...
                <div className="p-4 md:p-8 lg:p-10 max-w-7xl mx-auto w-full space-y-8">
                    ...
                    {vistaActiva === 'analiticas' && (
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 text-center py-20 text-gray-500">
                            <h2 className="text-3xl font-black text-gray-800 mb-2">Analíticas Globales</h2>
                            <p>Módulo de métricas en construcción.</p>
                        </div>
                    )}
                    ...
                    {vistaActiva === 'personal' && (
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-6 gap-4">
                                ...
                                <button
                                    onClick={() => { setMostrarModalUsuario(true); setCredencialesGeneradas(null); }}
                                    className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3.5 bg-[#F26522] hover:bg-[#d9551c] text-white font-bold rounded-2xl shadow-lg shadow-orange-500/30 transition-all transform active:scale-95"
                                >
                            ...
                            <div className="bg-white rounded-3xl md:rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden min-h-[300px]">
                                ...
                                ${usuario.rol === 'coordinador' ? 'bg-purple-100 text-purple-700' :
                                                                    usuario.rol === 'prefecto' ? 'bg-blue-100 text-blue-700' :
                                                        ...
                                                        <td className="p-5 text-right">
                                                            <button className="text-[#008542] hover:text-[#005a2d] font-bold text-sm transition-colors bg-green-50 px-3 py-1.5 rounded-lg hover:bg-green-100">Ver</button>
                                                        </td>
                                                    </tr>
                                                )) : (
                                                    <tr><td colSpan="4" className="text-center py-10 text-gray-500">No hay personal registrado en la base de datos.</td></tr>
                                                )}
                                            </tbody>
                                        </table>
                                    </div>
            ...
            {mostrarModalUsuario && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-[2rem] p-8 w-full max-w-md shadow-2xl m-auto relative overflow-hidden">
                        ...
                        {/* Si ya se generó la credencial, mostramos el componente de Éxito en lugar del Formulario */}
                        {credencialesGeneradas ? (
                            <div className="text-center animate-in zoom-in duration-300">
                                ...
                                <div className="bg-gray-50 p-5 rounded-2xl text-left border border-gray-200 space-y-4">
                                    <div>
                                        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Correo de Acceso</p>
                                        <p className="font-mono text-gray-800 font-bold">{credencialesGeneradas.usuario}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Contraseña</p>
                                        <p className="font-mono text-[#F26522] font-bold">{credencialesGeneradas.password}</p>
                                    </div>
                                </div>

                                <button
                                    onClick={() => { setMostrarModalUsuario(false); setCredencialesGeneradas(null); }}
                                    className="w-full mt-8 py-4 bg-[#008542] text-white font-bold rounded-xl hover:bg-[#005a2d] shadow-lg transition-all active:scale-95"
                                >
                                    Cerrar y Entendido
                                </button>
                            </div>
                            ...
                            <div className="animate-in fade-in">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input value={nuevoNombre} onChange={e => setNuevoNombre(e.target.value)} type="text" className="w-full p-3.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:bg-white focus:border-[#008542]" placeholder="Ej. Roberto Sánchez" />
                                    </div>

                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 mb-1.5">Usuario (Acceso)</label>
                                        <input value={nuevoUsuario} onChange={e => setNuevoUsuario(e.target.value)} type="text" className="w-full p-3.5 bg-gray-50 border border-gray-200 rounded-xl outline-none lowercase focus:bg-white focus:border-[#008542]" placeholder="ej. roberto.sanchez" />
                                        <p className="text-xs text-gray-400 mt-1">Se creará como: <strong className="text-[#008542]">{nuevoUsuario ? `${nuevoUsuario.trim().toLowerCase()}@cecyteh.local` : '...'}</strong></p>
                                    </div>

                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 mb-1.5">Contraseña Temporal</label>
                                        <input value={nuevaPassword} onChange={e => setNuevaPassword(e.target.value)} type="text" className="w-full p-3.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:bg-white focus:border-[#008542]" placeholder="Mínimo 6 caracteres" />
                                    </div>

                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 mb-1.5">Rol en el Sistema</label>
                                        <select value={nuevoRol} onChange={(e) => setNuevoRol(e.target.value)} className="w-full p-3.5 bg-gray-50 border border-gray-200 rounded-xl outline-none font-bold text-gray-700">
                                    ...
                                    {nuevoRol === 'coordinador' && (
                                        <div className="bg-purple-50 p-5 rounded-2xl border border-purple-100 animate-in zoom-in duration-200">
                                            ...
                                            <select value={nuevaCarreraId} onChange={e => setNuevaCarreraId(e.target.value)} className="w-full p-3 bg-white border border-purple-200 rounded-xl outline-none font-bold text-gray-700">
                                                {carrerasDB.length > 0 ? carrerasDB.map(c => (
