...
export default function PrefectoDashboard() {
    ...
    const seleccionarAlumno = async (alumnoBase) => {
        ...
        // Ordenamos por la fecha de incidencia en lugar de la del sistema
        ...
        // Ordenamos por la fecha del abono en lugar de la del sistema
        ...
        const historialFormateado = reportesData.map(rep => ({
            ...
            // Mostramos la fecha del suceso, o la de creación si es un dato muy viejo
            fecha: rep.fecha_incidencia || new Date(rep.fecha_creacion).toISOString().split('T')[0],
    ...
    const guardarReporte = async () => {
        ...
        const { error } = await supabase.from('reportes').insert([{
            ...
            fecha_incidencia: fechaReporte, // La fecha elegida del calendario
            fecha_creacion: new Date().toISOString(), // La hora exacta del registro en la app
    ...
    const registrarAbono = async () => {
        ...
        // Obtenemos la hora real para el sistema
        const horaRegistroSistema = new Date().toISOString();

        const gravesPendientes = alumnoSeleccionado.reportesRaw
            .filter(r => r.horas_asignadas > 0 && r.estado_reporte === 'pendiente')
            .sort((a, b) => new Date(a.fecha_incidencia || a.fecha_creacion) - new Date(b.fecha_incidencia || b.fecha_creacion));
        ...
        for (let reporte of gravesPendientes) {
            ...
            await supabase.from('historial_horas').insert([{
                ...
                // SEPARACIÓN DE FECHAS:
                fecha_abono: fechaAbono, // Calendario
                fecha_registro: horaRegistroSistema // Momento exacto en la app
            }]);
            horasRestantes -= pago;
        }
        ...
        if (horasRestantes > 0) {
            ...
            while (horasRestantes > 0 && (i + 2) < menoresPendientes.length) {
                ...
                await supabase.from('historial_horas').insert([{
                    ...
                    fecha_registro: horaRegistroSistema
                }]);
                                                        ...
                                                        historialAbonos.map((abono) => (
                                                            <div key={abono.id} className="bg-green-50/50 p-4 md:p-5 rounded-2xl border border-green-100 flex justify-between items-center gap-3">
                                                                <div>
                                                                    ...
                                                                    <p className="text-xs text-gray-400 font-medium mt-1">{abono.fecha_abono || new Date(abono.fecha_registro).toISOString().split('T')[0]}</p>
                                                                </div>
                                                                <div className="text-right shrink-0">
